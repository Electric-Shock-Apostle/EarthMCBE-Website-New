...
+ `/lang` Open language select GUI

  English/Chinese

# Money

+ `/money` Display your account's current balance.
+ `/money pay <amount> <player>` Pay an amount to a player. The transaction will only succeed if your account has at least the given amount plus any taxes that apply, and the receiving account has enough capacity for the amount.
+ `/money withdraw <amount>`  Withdraw an amount from chest storage into inventory.
+ `/money deposit <amount>`  Deposit an amount from inventory into chest storage.

# Vault

**Put the sign on the chest, not above it**

+ Player vaults: First line: `[vault]` Third line: will display your name on successful creation.
+ Towny vaults: 
...
**Holding the item that you want to sell in your hand and click a chest, then follow the instructions to create a shop**

+ `/qs setowner` Changes shop owner to <player>.
...
+ `/qs find <item>` Use to find the nearest shop that begins with <item> - example: `/qs find dia` will find the nearest diamond shop.
+ `/qs fetchmessage` Fetch shop message manually from the database
+ `/qs create` Create a shop without clicking the chest
+ `/qs staff` Manage staff to your shop
  - `/qs staff add <player>` add staff.
  - `/qs staff del <player>` delete staff
  - `/qs staff clear` remove all staff
  - `/qs staff list` show shop's staff
+ `/qs item` Change the shop trading item
